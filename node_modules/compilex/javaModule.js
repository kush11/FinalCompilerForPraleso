...
exports.compileJava = function (envData, code, fn) {
	...
	fs.mkdir(path, 0777, function (err) {
		if (err && exports.stats)
			...
			fs.writeFile(path + "/Praleso.java", code, function (err) {
				if (err && exports.stats)
					console.log('ERROR: '.red + err);
				else {
					if (exports.stats)
						console.log('INFO: '.green + path + "/Praleso.java created");
					...
					exec(command, function (error, stdout, stderr) {
						if (error) {
							if (exports.stats)
								console.log("INFO: ".green + path + "/Praleso.java contained an error while compiling");
							...
							var command = "cd " + path + " && java Praleso";
							exec(command, function (error, stdout, stderr) {
								if (error) {
									...
									if (error.toString().indexOf('Error: stdout maxBuffer exceeded') != -1) {
										...
										if (exports.stats) {
											console.log('INFO: '.green + path + '/Praleso.java contained an error while executing');
										}
										var out = { error: stderr };
										deleteData(path);
										fn(out);
									...
									if (exports.stats) {
										console.log('INFO: '.green + path + '/Praleso.java successfully compiled and executed !');
									}
						...
						if (err && exports.stats)
							...
							exec(command, function (error, stdout, stderr) {
								if (error) {
									...
									console.log("INFO: ".green + "compiled a java file");
									var command = "cd " + path + " && java Praleso < input.txt";
									exec(command, function (error, stdout, stderr) {
										if (error) {
...
exports.compileJavaWithTestCase = function (envData, code, userName, language, moduleId, fn) {
			...
			fs.writeFile(path + '/Praleso.java', code, function (err) {
				if (exports.stats) {
					if (err)
						...
						var tempcommand = "cd temp" + "/" + userName + " && " + " java Praleso";
